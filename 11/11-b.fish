...
function lcm
	...
	set min (math min (string join ',' $argv))
	set lcm $max

	while test (math $lcm % $min) -ne 0
		set lcm (math $lcm + $max)
	end

	echo $lcm
end

function monkey_debug
	for i in (seq 0 $ALPHA_MONKEY)
		echo "$i : $$i"
	end
end

function monkey_business
...
for line in $input
	if string match -r -q 'Monkey' $line
		...
		set calc $calc (string split ' ' $line)[-1]
	end
end
